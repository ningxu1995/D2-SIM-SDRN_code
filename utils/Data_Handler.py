...
def get_data_loader(opt):
    # dataloaders
    data_loader = load_dataset(opt.root, 'train', opt)
    valid_loader = load_dataset(opt.root, 'valid', opt)
...
class LatticeSIMDataset(Dataset):
    ...
    def __init__(self, root, category, opt):
        ...
        random.seed(1234)    # train and test datasets do not overlap
        random.shuffle(self.images)

        if category == 'train':
            self.images = self.images[:opt.n_train]
        else:
            self.images = self.images[-opt.n_test:]
...
def load_dataset(root, category, opt):
    dataset = LatticeSIMDataset(root, category, opt)
    if category == 'train':
        data_loader = DataLoader(dataset, batch_size=opt.batch_size, shuffle=True, num_workers=opt.workers)
    else:
        data_loader = DataLoader(dataset, batch_size=opt.batch_size_test, shuffle=False, num_workers=0)
    return data_loader
