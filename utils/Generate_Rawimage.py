...
lattice = 'hexagonal_add'  # hexagonal_add, hexagonal_mul, square
data = 'test'

# read images
out_dir = "Training_data/SIMdata1_DIV2K"
os.makedirs(out_dir, exist_ok=True)
# return all .png files
files = glob.glob("Training_data/DIV2K_subset/*.png")
# files = glob.glob("Training_data/DIV2K_train_HR/*.png")
nrep = 1  # repeat images
save_pos = True  # save positions
...
def get_params():
    ...
    # system
    opt.na = 1.45 * (1+ 0.01 * (np.random.rand() - 0.5))
    # parameter of camera noise_a=QE*AD noise_b=AD^2*sigma^2 refer to
    # "Practical Poissonian-Gaussian noise modeling and fitting for single-image raw-data"
    opt.noise_a = 2 * (1 + 0.01 * (np.random.rand() - 0.5))    # 1.74
    opt.noise_b = 1 * 31.9 * (1 + 0.01 * (np.random.rand() - 0.5))
    opt.m_value = 1000 * (1 + 0.01 * (np.random.rand() - 0.5))
    opt.background = 100 * (1 + 0.1 * (np.random.rand() - 0.5))
    # lattice parameter
    opt.scale = 0.75 * (1 + 0.01 * (np.random.rand() - 0.5))  # fp is the scale times of fco
    opt.mean_intensity = 0.5  # average intensity
    opt.mod_fac = 0.8 + 0.03 * (np.random.rand() - 0.5)  # modulation factor
    opt.disp_err = 0.01 * (np.random.rand(num ** 2, 2) - 0.5)
    # opt.rotate_angle = 0 * pi * (np.random.rand() - 0.5)
    opt.rotate_angle = 0
    return opt
...
def generate_sim_image(ima, opt):
    ...
    if 'exp' in lattice:
        fp = 0.75 * fco / 2
        p_pattern = io.imread("C:\\Users\\86158\\Pictures\\ml\\hexadd_exp.png")/255
    elif 'simu' in lattice:
        fp = 0.75 * fco / 2
        p_pattern = io.imread("C:\\Users\\86158\\Pictures\\ml\\hexadd_simu.png")/255
    else:
        if 'square' in lattice:
            fp = opt.scale * fco/sqrt(2)
            # a_ideal = 1/2*(cos(2 * pi * fp * (sqrt(2)/2*x+sqrt(2)/2*y)) + cos(2 * pi * fp * (sqrt(2)/2*x-sqrt(2)/2*y)))
            a_ideal = 1/2*(cos(2*sqrt(2)*pi*fp*(cos(opt.rotate_angle+pi/4)*x+sin(opt.rotate_angle+pi/4)*y)) + cos(
                2*sqrt(2)*pi*fp*(cos(opt.rotate_angle+3*pi/4)*x+sin(opt.rotate_angle+3*pi/4)*y)))
            p_pattern = opt.mean_intensity + opt.mod_fac * (a_ideal ** 2 - 0.5)
        elif 'hexagonal_mul' in lattice:
            fp = opt.scale * fco / 2
            a_ideal = cos(2 * pi * fp * y) * cos(2 * pi * fp * (y / 2 + sqrt(3) / 2 * x)) * cos(
                2 * pi * fp * (y / 2 - sqrt(3) / 2 * x))
            p_pattern = opt.mean_intensity + opt.mod_fac * (a_ideal ** 2 - 0.5)
        elif 'hexagonal_add' in lattice:
            fp = opt.scale * fco/2
            # a_ideal = 1/3*(cos(4 * pi * fp * y) + cos(4 * pi * fp * (y / 2 + sqrt(3) / 2 * x)) + cos(
            #     4 * pi * fp * (y / 2 - sqrt(3) / 2 * x)))
            a_ideal = 1/3*(cos(4 * pi * fp * (cos(opt.rotate_angle+pi/2)*x+sin(opt.rotate_angle+pi/2)*y)) + cos(
                4 * pi * fp * (cos(opt.rotate_angle+pi/6)*x+sin(opt.rotate_angle+pi/6)*y)) + cos(
                4 * pi * fp * (cos(opt.rotate_angle+5*pi/6)*x+sin(opt.rotate_angle+5*pi/6)*y)))
            p_pattern = opt.mean_intensity + opt.mod_fac * (a_ideal ** 2 - 0.5)
    ...
    wide_image = (wide_image - np.min(wide_image)) / np.max(wide_image) * opt.m_value
    ...
    n_wide_image = wide_image + noise_frac * noise_w + opt.background
    ...
    if data == 'test':
        ...
        raw_image = (raw_image - np.min(raw_image)) / np.max(raw_image)
        raw_image = raw_image * opt.m_value
        sigma_noise = sqrt(opt.noise_a * raw_image + opt.noise_b)
        noise_r = np.random.normal(0, 1, (nx, ny)) * sigma_noise
        n_raw_image = raw_image + noise_frac * noise_r + opt.background

        # snr = np.sum(raw_image ** 2) / np.sum(noise_r ** 2)
        # print(snr)
