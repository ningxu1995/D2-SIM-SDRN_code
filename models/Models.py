...
def get_discriminator_model(opt):

    net = VggDiscriminator(opt.nch_out, 1, 64)
    if not opt.cpu:
        net.cuda()
        net = nn.DataParallel(net)
...
class EDSR(nn.Module):
    def __init__(self, opt):
        ...
        n_res_blocks = opt.n_res_blocks  # originally 16
...
class ResidualGroup(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, act, res_scale, n_res_blocks):
...
class RCAN(nn.Module):
    def __init__(self, opt):
        ...
        n_res_groups = opt.n_res_groups
        n_res_blocks = opt.n_res_blocks
        n_feats = opt.n_feats
        kernel_size = 3
        reduction = opt.reduction
        act = nn.ReLU(True)
        self.n_arch = opt.n_arch
        ...
        modules_body = [
            ResidualGroup(
                conv, n_feats, kernel_size, reduction, act=act, res_scale=1, n_res_blocks=n_res_blocks)
...
class VggDiscriminator(nn.Module):
    def __init__(
        ...
        self.body = nn.Sequential(
            DownSample(channels, channels,2,1),
            DownSample(channels, channels*2, 1, 1),
            DownSample(channels*2, channels*2, 2, 1),
            DownSample(channels*2, channels*4, 1, 1),
            DownSample(channels*4, channels*4, 2,1),
            DownSample(channels*4, channels*8, 1, 1),
            DownSample(channels*8, channels*8, 2, 1),
        )
        self.tail = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(channels*8, channels*16, 1),
            nn.LeakyReLU(inplace=True),
...
class FRCAB(nn.Module):
    def __init__(
            self, conv, n_feat, kernel_size, reduction,
            bias=True, bn=False, act=nn.ReLU(True), res_scale=1):

        super(FRCAB, self).__init__()
        modules_body = []
        for i in range(2):
            modules_body.append(conv(n_feat, n_feat, kernel_size, bias=bias))
            if bn:
            ...
            if i == 0:
                modules_body.append(act)
        modules_body.append(FCALayer(n_feat, reduction))
        self.body = nn.Sequential(*modules_body)
        self.res_scale = res_scale
...
# Residual Group (RG)
class FResidualGroup(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, act, res_scale, n_res_blocks):
        super(FResidualGroup, self).__init__()
        # modules_body = []
        modules_body = [
            FRCAB(
                conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(True), res_scale=1)
...
# Residual Channel Attention Network (RCAN)
class FRCAN(nn.Module):
    def __init__(self, opt):
        super(FRCAN, self).__init__()
        n_res_groups = opt.n_res_groups
        n_res_blocks = opt.n_res_blocks
        n_feats = opt.n_feats
        kernel_size = 3
...
if __name__ == '__main__':
    net = VggDiscriminator(1, 1, 64)
    aa = torch.rand([2, 1, 512, 512])
    output = net(aa)
    print(aa.shape)
    print(output.shape)
